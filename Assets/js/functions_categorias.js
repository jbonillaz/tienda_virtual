...
// Extraer los datos de una categoria, para poder mostrarlos en el icono azul de ver categoria
    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
                ...
                $('#listStatus').selectpicker('render'); //para refrescar la opcion.
...
// Funcion para eliminar categorial
function fntDelInfo(idcategoria) {
    swal({
        ...
        if (isConfirm) {
            let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP'); //para la implementacion de AJAX
...
// Funcion que nos permite cancelar o eliminar desde la x roja la imagen cargada
